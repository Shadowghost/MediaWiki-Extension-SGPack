	...
	ParserOptionsRegisterHook
{
	 ...
	 * parser-cache entry. The fields that do differ between anonymous readers are
	 * handled in ParserAdds::sgPackUserInfo(), which opts those individual parses
	...
	public function onParserOptionsRegister( &$defaults, &$inCacheKey, &$lazyLoad ) {
		...
		$lazyLoad[ParserAdds::USER_PARSER_OPTION] = static function ( ParserOptions $options ) {
			...
			// Group membership belongs in the key as well as the name, because
			// {{userinfo:group}} and {{userinfo:groups}} change what they render when
			// a user is promoted or demoted. Sorted, so an ordering change in the
